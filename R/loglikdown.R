#' Compute the likelihood for the downstream model.
#'
#' @export
...
#' @return The value of the log likelihood for the downstream model.
#' @examples
...
#' par <- c(-2,1,0.3,-4,10)
#' loglik.down(wt,ko,par)

loglik.down <- function(wt,ko,par) {
  ...
  res <- res+sum(dnorm(ko[,2],alpha*ko[,1]+mu[2],sigma[2],log <- TRUE))
  return(-res)
}
